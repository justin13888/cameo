...
fn days_ago(n: i64) -> String {
    ...
    (Utc::now().date_naive() - Duration::days(n)).format("%Y-%m-%d").to_string()
}
...
fn movie_old_released_gets_long_ttl() {
    ...
    assert_eq!(ttl, cfg.old_content_details_ttl, "old released movie should use old_content_details_ttl");
}
...
fn movie_recent_gets_medium_ttl() {
    ...
    let ttl = cfg.movie_details_ttl(Some(&release), Some("Released"));
    assert_eq!(ttl, cfg.recent_content_details_ttl, "recently released movie should use recent_content_details_ttl");
}
...
fn movie_in_production_gets_medium_ttl() {
    ...
    assert_eq!(ttl, cfg.recent_content_details_ttl, "in-production movie should use recent_content_details_ttl");
}
...
fn movie_released_mid_range_falls_back_to_details_ttl() {
    ...
    let ttl = cfg.movie_details_ttl(Some(&release), Some("Released"));
    assert_eq!(ttl, cfg.details, "mid-range released movie should fall back to details TTL");
}
...
fn tv_active_in_production_gets_short_ttl() {
    ...
    assert!(ttl <= Duration::from_secs(4 * 3600), "active TV show must be cached for ≤4h");
}
...
fn tv_recently_aired_gets_short_ttl() {
    ...
    let ttl = cfg.tv_show_details_ttl(Some(&days_ago(2000)), Some(&last_air), Some("Returning Series"), false);
    assert_eq!(ttl, cfg.active_content_details_ttl);
...
fn tv_old_ended_gets_long_ttl() {
    ...
    assert_eq!(ttl, cfg.old_content_details_ttl, "old ended show should use old_content_details_ttl");
}
...
fn tv_returning_series_gets_medium_ttl() {
    ...
    let ttl = cfg.tv_show_details_ttl(Some(&first_air), Some(&last_air), Some("Returning Series"), false);
    assert_eq!(ttl, cfg.recent_content_details_ttl);
}
...
fn tv_active_overrides_old_status() {
    ...
    assert_eq!(ttl, cfg.active_content_details_ttl, "in_production=true overrides old/ended status");
}
