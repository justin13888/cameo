...
use cameo::providers::tmdb::image_url::{BackdropSize, ImageUrl, PosterSize};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !movie.genres.is_empty() {
        let genre_names: Vec<_> = movie.genres.iter().filter_map(|g| g.name.as_deref()).collect();
    ...
    if let Some(backdrop) = &movie.backdrop_path {
        println!("Backdrop: {}", ImageUrl::backdrop(backdrop, BackdropSize::W1280));
    }
    ...
    println!("\nTop Crew:");
    for member in credits.crew.iter().filter(|c| matches!(c.job.as_deref(), Some("Director") | Some("Producer") | Some("Screenplay"))).take(5) {
